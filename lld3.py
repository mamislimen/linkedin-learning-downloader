...
class Lld:
    ...
    def format_string(raw_string):
        replacement_dict = {u'Ä': 'Ae', u'Ö': 'Oe', u'Ü': 'Ue', u'ä': 'ae', u'ö': 'oe', u'θ': '', u'ü': 'ue', ':': '-','?': ''}
        invalid_chars = r'[^A-Za-z0-9\-\.\'éàèçî]+'
        u_map = {ord(key): val for key, val in replacement_dict.items()}
        raw_string = raw_string.translate(u_map)
        raw_string = re.sub(invalid_chars, ' ', raw_string).strip()
        i = 0
        for c in raw_string:
    ...
    def download_sub(self, subs, path, file_name):
        with open(path + '/' + file_name, 'a',errors="ignore") as f:
            i = 1
            for sub in subs:
